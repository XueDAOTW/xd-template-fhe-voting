...
import {
  useEthersProvider,
  useEthersSigner,
} from "../utils/viemEthersConverters"; // Adjust the import path as needed
...
export default function LandingPage() {
    ...
    try {
      if (element !== null) {
        ...
        if (isNaN(value)) {
          ...
          const uint8Array = await client.encrypt_uint8(value);
          console.log("Encrypted uint8Array:", uint8Array);
          return uint8Array;
      }
    } catch (err) {
  ...
  useEffect(() => {
    const fetchProposalName = async () => {
      try {
        ...
        console.error("Error fetching proposal name:", error);
      }
    };
    fetchProposalName();
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    const encryptedVote = await encrypt(selectedOption).catch((error) => {
          ...
          <>
            <div className="w-full max-w-md bg-gray-100 p-6 mt-10 rounded-lg shadow-md">
              ...
              <p className="text-center text-gray-700 font-bold">
                The Winning Character is: {options[winningValues.uint8Value]} 🎉
              </p>
            </div>
            <div className="py-20">
              <img
          ...
          <div className="flex space-x-5">
            ...
            <div className="rounded-xl text-xs border border-slate-500 bg-gradient-to-b from-zinc-800/30 to-zinc-500/40 p-4 flex flex-col">
              ...
              <div className="flex items-center justify-center space-x-2">
                ...
                <p>
                  {blockNumberData ? Number(blockNumberData) : "Loading..."}
                </p>
              </div>
            </div>
            {!finalized && (
